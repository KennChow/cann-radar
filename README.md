...
| 升级机制 | 首次提醒本人；≥7 个工作日后仍 open 则二次提醒本人；最多 2 次，之后标记为"需管理员介入" | 同 MR |
...
每天 22:00，`admin_summary.py` 读取 MR 和 Issue 扫描结果，按仓库独立发送一封合并邮件（Issue 表在上、MR 表在下），收件人为该仓库的 `admin`，抄送 `cc`。
